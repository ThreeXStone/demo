...
export default function DynamicForm({ title, fields, submitLabel, submittedFormData, onAction }: Props) {
  ...
  const readonlyClass = 'w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm text-gray-600';
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
      ...
      <div className="p-4 space-y-4">
        {fields.map((f) => {
          const savedValue = submittedFormData?.[f.name] ?? '';
          if (hasSubmitted) {
            return (
              <div key={f.name}>
                <label className={labelClass}>{f.label}</label>
                <div className={readonlyClass}>{savedValue || '-'}</div>
              </div>
            );
          }
          return (
